...
class FCWorkflowActions
{
		   ...
		   is_admin() && preg_match_all('/page=oasiswf(.*)|post-new\.(.*)|post\.(.*)/', $_SERVER['REQUEST_URI'], $matches )){
			...
			if ( !$is_post_published ) {
				wp_enqueue_script( 'owf_submit_workflow',
								   ...
								   OASISWF_VERSION,
								   true);
				FCProcessFlow::enqueue_acf_validator_script();
   ...
   {
      global $wpdb;
      if( !class_exists('FCUtility') ){
      	require_once( OASISWF_PATH . "oasiswf-utilities.php" );
      }
      ...
      if( !class_exists('FCWorkflowEmail') ){
      	require_once( OASISWF_PATH . "includes/workflow-email.php" ) ;
      }
      $emails_table = FCUtility::get_emails_table_name();
