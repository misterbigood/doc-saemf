...
<div id="reassgn-setting" class="popup-div">
			...
			<?php
				foreach ($users as $user) {
					...
					if( $task_user != $user->ID )
						echo "<option value='".esc_attr( $user->ID )."'>$lblNm</option>" ;
				}
			?>
		</select>
	</p>
	...
	<input type="hidden" id="action_history_id" name="action_history_id" value=<?php echo esc_attr( $history_id );?> />
	<input type="hidden" id="task_user_inbox" name="task_user_inbox" value=<?php echo esc_attr( $task_user );?> />
	<br class="clear">
</div>
