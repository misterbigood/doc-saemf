					...
					if($workflows):
						...
						foreach ($workflows as $wf){
							...
							if ( $count >= $start )
							{
								...
								if ( ! $postcount && $wfcount->wfall > 1) {
									$delete_nonce = wp_create_nonce('workflow-delete-nonce');
									echo "&nbsp;|&nbsp;<span><a href='admin.php?page=oasiswf-admin&wf_id=" . $wf->ID . "&action=delete&_nonce=$delete_nonce'>" . __( "Delete", "oasisworkflow" ) . "</a></span>";
								}								
								echo "</div>
									</td>";
