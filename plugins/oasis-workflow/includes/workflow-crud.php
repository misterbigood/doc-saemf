...
class FCWorkflowCRUD extends FCWorkflowBase
{
	static function save()
	{
		...
		$wf = FCWorkflowBase::get_workflow_by_id( $workflow_id );		
		$parent_id= $wf->parent_id;
		if( $parent_id > 0 ) {
			$end_date = str_replace('-', '/', $startdate);
						...
						<tr height="50px">
							...
							if($oasiswf_path){
								foreach ($oasiswf_path as $k => $v) {
									$str .= '<td width="110px;">
												<input type="radio" id="path-opt-' . $v[1] . '" name="path-opt" value="' . esc_attr( $v[1] ) . '" ' . '/> ' . $v[0] . '
											</td>';
								}
							}
				$str .=	'</tr>
	 ...
	 * @since 1.7
	 */
	static function delete_workflow()
	{
		global $wpdb;
	
		if ( ! wp_verify_nonce( $_GET['_nonce'], 'workflow-delete-nonce' ) ) {
			return;
		}
		...
		self::delete_workflow_steps( $wf_id ) ;
		...
		// redirect back the workflow list page
		wp_redirect( admin_url( 'admin.php?page=oasiswf-admin' ) );
		die();
	}
	
	/**
	 * Delete steps for a given workflow.
	 * 
	 * @since 1.7 initial version
	 */
	static function delete_workflow_steps( $wf_id )
	{
		global $wpdb;
		$wf = FCWorkflowList::get_workflow_by_id( $wf_id ) ;
		if( $wf ){
			$wfinfo = $wf->wf_info ;
			if( $wfinfo ) {
